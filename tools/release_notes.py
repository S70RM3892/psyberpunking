...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "### Drive（仮）— ゲームパッドで街を走る（別アプリ）",
        "",
        "ベンチと同じ街を自分で運転する。`citydrive-*-arm64" + ("-debugkey" if args.signing == "debug" else "") + ".apk` を"
        "同じ手順でインストール（ベンチとは別のアプリとして並んで入る）。Bluetooth のゲームパッドが必要。",
        "",
        "| 操作 | ボタン |",
        "| --- | --- |",
        "| ハンドル | 左スティック |",
        ...
        "| サイドブレーキ（ドリフト） | A |",
        "| 視点の切り替え / 道路に戻す | Y / B |",
        "| 見回す / ヘッドライト / 一時停止 | 右スティック / SELECT / START |",
        "",
        "Linux 版は tar の中の `./citydrive`（キーボードでも走れる：WASD・Space・C・R）。",
        "",
        "### 既知の問題",
        "",
        *[f"- {k}" for k in known],
