...
// ドライブ（ベンチとは別アプリ）。街・描画・マテリアル・シーン定義はベンチと共有し、ネイティブは drive/ も含めてビルドする。
// versionName はアプリの版、versionCode は単調増加の整数（CIの実行番号）
val appVersionName: String = (findProperty("drive.versionName") as String?) ?: "0.1.0"
val appVersionCode: Int = ((findProperty("drive.versionCode") as String?) ?: "1").toInt()

android {
    namespace = "dev.zat.citydrive"
    compileSdk = 36
    ndkVersion = "29.0.14206865"

    defaultConfig {
        applicationId = "dev.zat.citydrive"
        minSdk = 30        // ベンチと同じ下限（Vulkan 1.1・16KBページ対応の端末）
